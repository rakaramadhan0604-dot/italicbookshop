...
$oldName   = htmlspecialchars($old['fullname'] ?? '', ENT_QUOTES, 'UTF-8');
$oldEmail  = htmlspecialchars($old['email'] ?? '', ENT_QUOTES, 'UTF-8');
$oldPhone  = htmlspecialchars($old['phone'] ?? '', ENT_QUOTES, 'UTF-8');
$oldKtp    = htmlspecialchars($old['no_ktp'] ?? '', ENT_QUOTES, 'UTF-8');
$oldAlamat = htmlspecialchars($old['alamat'] ?? '', ENT_QUOTES, 'UTF-8');
?>
<!DOCTYPE html>
<html lang="id">
  <head>
    ...
    <meta name="description" content="Buat akun Italic gratis untuk mulai menyewa buku." />

    <!-- Open Graph / social sharing -->
    <meta property="og:type" content="website" />
    <meta property="og:site_name" content="Italic" />
    <meta property="og:title" content="Daftar — Italic" />
    <meta property="og:description" content="Buat akun Italic gratis untuk mulai menyewa buku." />
    <meta property="og:url" content="https://www.italic.co.id/signup.php" />
    <meta property="og:image" content="https://www.italic.co.id/assets/og-image.jpg" />
    <meta property="og:image:width" content="1200" />
    <meta property="og:image:height" content="630" />
    <meta property="og:locale" content="id_ID" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="Daftar — Italic" />
    <meta name="twitter:description" content="Buat akun Italic gratis untuk mulai menyewa buku." />
    <meta name="twitter:image" content="https://www.italic.co.id/assets/og-image.jpg" />
    <title>Daftar — Italic</title>
    ...
    <main id="main">
      <div class="formsheet">
        <section class="card">
          ...
          <header class="card-title">Daftar Akun Italic</header>
          <p class="card-sub">Gratis, dan hanya butuh satu menit untuk mulai menyewa buku pertamamu.</p>

          <?php if (!empty($signupErrors)): ?>
          <div class="form-status err">
            <?php foreach ($signupErrors as $err): ?>
            <p style="margin:0;"><?php echo htmlspecialchars($err, ENT_QUOTES, 'UTF-8'); ?></p>
            <?php endforeach; ?>
          </div>
          <?php endif; ?>
          ...
          <form action="process/register.php" method="POST" novalidate>
            <div class="field">
              ...
              <input type="text" id="signup-name" name="fullname" value="<?php echo $oldName; ?>" required />
            </div>
            <div class="field-row">
              <div class="field">
                <label for="signup-email">Email</label>
                <input type="email" id="signup-email" name="email" value="<?php echo $oldEmail; ?>" required />
              </div>
              <div class="field">
                <label for="signup-phone">No. HP</label>
                <input type="tel" id="signup-phone" name="phone" placeholder="08xx-xxxx-xxxx" value="<?php echo $oldPhone; ?>" required />
              </div>
            </div>
            <div class="field">
                <label for="signup-ktp">Nomor KTP</label>
                <input
                    type="text"
                    id="signup-ktp"
                    name="no_ktp"
                    ...
                    inputmode="numeric"
                    maxlength="16"
                    value="<?php echo $oldKtp; ?>"
                    required
                />
            </div>
            <div class="field">
              <label for="signup-password">Kata Sandi</label>
              <input type="password" id="signup-password" name="password" placeholder="Minimal 8 karakter" minlength="8" required />
            </div>
            <div class="field">
                <label for="signup-address">Alamat</label>
                <textarea
                    id="signup-address"
                    name="alamat"
                    rows="3"
                    required
                ><?php echo $oldAlamat; ?></textarea>
            </div>
            <button type="submit" class="btn btn-primary btn-block">Buat Akun</button>
            <p class="form-status" aria-live="polite"></p>
          </form>
          <p class="helper-link">Sudah punya akun? <a href="login.php">Masuk di sini</a></p>
        </section>
      </div>
    </main>
